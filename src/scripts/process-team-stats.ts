...
function aggregateBattingStats(
  ...
  const result = new Map<string, TeamBattingStats>()

  for (const [key, data] of grouped) {
    const denominator = data.sumAB + data.sumBB + data.sumHP + data.sumSF
    const obp =
      denominator > 0 ? (data.sumH + data.sumBB + data.sumHP) / denominator : 0
    ...
    result.set(key, {
      year: data.year,
      team: data.team,
      obp: Number(obp.toFixed(4)),
      slg: Number(slg.toFixed(4)),
...
function aggregatePitchingStats(
  ...
  const result = new Map<string, TeamPitchingStats>()
...
function calculateDerivedStats(
  battingStats: Map<string, TeamBattingStats>,
  pitchingStats: Map<string, TeamPitchingStats>
): TeamSeasonStats[] {
  ...
  const byYear = new Map<number, TeamSeasonStats[]>()
